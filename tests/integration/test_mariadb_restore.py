"""Integration tests for MariaDB restore operations.

Each test first runs a backup via BackupOrchestrator, then restores from
the created backup using Restorer. Tests run against real MariaDB containers
...
BACKUP_CONNECTION = "test-mariadb-restore"
...
def test_restore_full(mariadb_instance: DBInstance, backup_output_dir: Path):
    """Full backup then full restore with --drop-databases succeeds.

    Verifies data actually exists in restored tables.
    """
    backup_dir = _do_full_backup(mariadb_instance, backup_output_dir)
...
def test_restore_specific_database(
    mariadb_instance: DBInstance, backup_output_dir: Path
):
...
def test_restore_globals_only(mariadb_instance: DBInstance, backup_output_dir: Path):
...
def test_restore_dry_run(mariadb_instance: DBInstance, backup_output_dir: Path):
...
def test_restore_log_created(mariadb_instance: DBInstance, backup_output_dir: Path):
...
def test_restore_exit_code_0(mariadb_instance: DBInstance, backup_output_dir: Path):
