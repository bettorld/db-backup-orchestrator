"""Integration tests for MySQL backup operations.

Tests run against real MySQL containers (parametrized across versions).
...
def _make_backup_config(
    ...
    *,
    ...
    # MySQL uses root user for full dump capabilities
    return BackupConfig(
...
def test_full_backup(mysql_instance: DBInstance, backup_output_dir: Path):
    """Full backup creates globals + all database subfolders + manifest success."""
    config = _make_backup_config(mysql_instance, backup_output_dir, full=True)
    ...
    assert manifest["driver"] == "mysql"


def test_full_backup_compressed(mysql_instance: DBInstance, backup_output_dir: Path):
...
def test_full_backup_no_compress(mysql_instance: DBInstance, backup_output_dir: Path):
    """--no-compress produces raw .sql files."""
    config = _make_backup_config(
        mysql_instance, backup_output_dir, full=True, no_compress=True
    )
    exit_code = _run_backup(config)
    assert exit_code == 0
...
def test_specific_databases(mysql_instance: DBInstance, backup_output_dir: Path):
    """--databases app_store backs up only that database."""
    config = _make_backup_config(
        mysql_instance, backup_output_dir, databases=["app_store"]
    )
    exit_code = _run_backup(config)
    assert exit_code == 0
...
def test_databases_only(mysql_instance: DBInstance, backup_output_dir: Path):
    """--databases-only auto-discovers all databases without globals."""
    config = _make_backup_config(mysql_instance, backup_output_dir, databases_only=True)
    exit_code = _run_backup(config)
    assert exit_code == 0
...
def test_globals_only(mysql_instance: DBInstance, backup_output_dir: Path):
    """--globals-only produces only globals, no database subfolders."""
    config = _make_backup_config(mysql_instance, backup_output_dir, globals_only=True)
    exit_code = _run_backup(config)
    assert exit_code == 0
...
def test_manifest_success(mysql_instance: DBInstance, backup_output_dir: Path):
    ...
    assert manifest["driver"] == "mysql"
    assert manifest["driver_version"] == mysql_instance.version
...
def test_dry_run(mysql_instance: DBInstance, backup_output_dir: Path):
    """--dry-run creates no files or directories."""
    config = _make_backup_config(
        mysql_instance, backup_output_dir, full=True, dry_run=True
    )
    exit_code = _run_backup(config)
    assert exit_code == 0
...
def test_exit_code_0(mysql_instance: DBInstance, backup_output_dir: Path):
...
def test_exit_code_1_bad_host(mysql_instance: DBInstance, backup_output_dir: Path):
...
def test_counter_increments(mysql_instance: DBInstance, backup_output_dir: Path):
    """Running backup twice creates .001 and .002 directories."""
    config1 = _make_backup_config(mysql_instance, backup_output_dir, globals_only=True)
    exit_code1 = _run_backup(config1)
    assert exit_code1 == 0

    config2 = _make_backup_config(mysql_instance, backup_output_dir, globals_only=True)
...
def test_schemas_ignored(mysql_instance: DBInstance, backup_output_dir: Path, caplog):
    """--schemas with MySQL logs a warning and ignores the filter.
    ...
    # After validation, schemas should be cleared for MySQL.
    # The backup should succeed normally.
    exit_code = _run_backup(config)
    assert exit_code == 0

    # Verify schemas was cleared (validation sets it to None for mysql)
