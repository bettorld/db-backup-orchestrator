"""Integration tests for MySQL restore operations.

Each test first runs a backup via BackupOrchestrator, then restores from
the created backup using Restorer. Tests run against real MySQL containers
(parametrized across versions).
"""
...
from tests.integration.helpers import query_mysql
...
BACKUP_CONNECTION = "test-mysql-restore"
...
def test_restore_full(mysql_instance: DBInstance, backup_output_dir: Path):
    """Full backup then full restore with --drop-databases succeeds.

    Verifies data actually exists in restored tables.
    """
    backup_dir = _do_full_backup(mysql_instance, backup_output_dir)
    ...
    count = query_mysql(
        mysql_instance,
        "SELECT COUNT(*) FROM app_store.products;",
    )
    assert int(count) > 0, (
        f"Expected data in app_store.products after restore, got {count}"
    )


def test_restore_specific_database(mysql_instance: DBInstance, backup_output_dir: Path):
...
def test_restore_globals_only(mysql_instance: DBInstance, backup_output_dir: Path):
    """Restore --globals-only restores only globals (users/grants)."""
    backup_dir = _do_full_backup(mysql_instance, backup_output_dir)
    ...
    # Verify that the globals user was created
    result = query_mysql(
        mysql_instance,
        "SELECT user FROM mysql.user WHERE user = 'app_readonly';",
    )
    assert "app_readonly" in result, (
        f"Expected 'app_readonly' user after globals restore, got: {result}"
    )


def test_restore_dry_run(mysql_instance: DBInstance, backup_output_dir: Path):
...
def test_restore_log_created(mysql_instance: DBInstance, backup_output_dir: Path):
...
def test_restore_exit_code_0(mysql_instance: DBInstance, backup_output_dir: Path):
...
def test_restore_no_drop_databases_fails(
    mysql_instance: DBInstance, backup_output_dir: Path
):
...
def test_restore_nonexistent_database(
    mysql_instance: DBInstance, backup_output_dir: Path
):
    ...
    # R6 validation catches that nonexistent_db is not in the manifest
